$(function() {
    ...
    Note.prototype = {
        ...
        createContainer: function() {
            ...
            var author = $('<span class="noteAuthor">').text(annotation.Author);
            var subject = $('<span class="noteSubject">').text(annotation.Subject);
            ...
            var time = $('<div class="noteTime">').text(moment(annotation.DateCreated).format(dateFormat));
            textCol.append(time);
            row.append(textCol);
            ...
            var editButton = $('<a class="notesLink" data-i18n="notesPanel.edit"></a>');
            editButton.on('click', function() {
                me.switchToEditingMode();
                me.trigger('update');
            });
            var deleteButton = $('<a class="notesLink" data-i18n="notesPanel.delete"></a>');
            deleteButton.on('click', function() {
                me.showDeleteConfirmation();
            });

            this.$editButtonsContainer.append(editButton);
            this.$editButtonsContainer.append(deleteButton);
        ...
        collapseContents: function() {
            ...
            if (this.$staticNoteContents.height() > this.COLLAPSE_HEIGHT) {
    ...
    NoteManager.prototype = {
        ...
        _createNote: function(annotation, imported) {
            ...
            if (annotation.isReply()) {
                ...
                note = new Note(annotation, editable, replyable);
                if (!imported) {
                ...
                note = new RootNote(annotation, editable, replyable);
            ...
            if (!imported && note instanceof RootNote) {
                ...
                if (annotation instanceof Annotations.StickyAnnotation) {
                    this.editNote(annotation, true);
                    this.selectNote(annotation);
                    this.focusAnnotation(annotation);
                }
            }

            readerControl.fireEvent('noteCreated', [note.getAnnotation(), note.getContainer()]);
        ...
        _forAllNotes: function(callback, filteredCallback, fromPages) {
            ...
            for (var annotId in this.notes) {
                ...
                if (filteredCallback && note.getAnnotation().Hidden) {
                    filteredCallback.call(this, note);
                    continue;
                }
        ...
        getAnnotationPosition: function(annotation, displayMode) {
            ...
            return displayMode.pageToWindow(new Annotations.Point(annotation.X + annotation.Width, annotation.Y), pageIndex);
        },
        ...
        forAllLines: function(callback, filteredCallback, fromPages) {
            for (var annotId in this.annotLines) {
                ...
                if (filteredCallback && annotLine.getAnnotation().Hidden) {
                    filteredCallback.call(this, annotLine);
                    continue;
                }
        ...
        selectNote: function(annotation) {
            var note = this._getNote(annotation);
            if (note) {
                note.select(this.annotManager.canModify(annotation));
            }
        },
        ...
        editNote: function(annotation, selectAll) {
            ...
            if (!this.annotManager.canModify(annotation)) {
                note.startAddingReply();
            } else {
                this._checkShouldSwitchSortMode(note);
        ...
        showFocusConnector: function() {
            if (!this.sortStrategy.showListView() && !this.annotManager.getMarkupOff()) {
    ...
    function resizePanel() {
        ...
        var availableWidth = window.innerWidth - $sidePanel.width();
        var maxWidth = Math.max(availableWidth * 0.75, MIN_PANEL_WIDTH);
    ...
    $document.on('viewerLoaded', function() {
        ...
        $noteOptions.find('#toggleAnnotationsButton').on('click', function() {
    ...
    $document.on('documentLoaded', function() {
        ...
        annotManager.on('annotationToggled', function(e, hidden) {
            if (hidden) {
