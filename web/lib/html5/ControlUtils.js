...
(function(exports) {
        ...
        function initializeReaderControl() {
            ...
            if (queryParams.getBoolean('startOffline')) {
                ...
                window.ControlUtils.byteRangeCheck(function(status) {
                    ...
                    window.readerControl.loadDocument(doc, {streaming: true});
                });
            }
        }
    ...
    exports.ControlUtils.registerTool = function(tool, name, annotType, annotationCheck) {
        /*jshint newcap:false */
        var toolObj = {
            tool: tool,
            name: name,
        ...
        var storedValue = exports.ControlUtils.importToolData(name);
    ...
    var getMatchingTools = function(annotation) {
        var matchedTools = [];

        var listLength = toolList.length;
        // check for matching tools that handle this type of annotation
        for (var i = 0; i < listLength; ++i) {
            if (toolList[i].annotationCheck(annotation)) {
                matchedTools.push(toolList[i]);
            }
        }
    ...
    exports.ControlUtils.getToolColors = function(annotation) {
        var matchedTools = getMatchingTools(annotation);
    ...
    exports.ControlUtils.getAdvancedToolColors = function() {
    ...
    function getColorList(annotation, property) {
        var colorList = null;
        var matchedTools = getMatchingTools(annotation);
        if (matchedTools.length === 1) {
    ...
    exports.ControlUtils.addToolColor = function(annotation, property, color) {
        var result = getColorList(annotation, property);
        ...
        if (colorList !== null) {
            if (colorList.indexOf(color) === -1) {
                ...
                exports.ControlUtils.exportToolData(result.tool);
            }
        }
    };

    exports.ControlUtils.removeToolColor = function(annotation, property, color) {
        var result = getColorList(annotation, property);
        var colorList = result.colorList;
        if (colorList !== null) {
    ...
    exports.ControlUtils.getDefaultToolEditingMode = function(annotation) {
        var matchedTools = getMatchingTools(annotation);
    ...
    exports.ControlUtils.setDefaultToolEditingMode = function(annotation, editMode) {
        var matchedTools = getMatchingTools(annotation);

        if (matchedTools.length === 1) {
            matchedTools[0].defaultEditMode = editMode;
            exports.ControlUtils.exportToolData(matchedTools[0]);
        }
    };
    ...
    function exportTool(tool) {
        ...
        localStorage['toolData-' + tool.name] = JSON.stringify(toolData);
    }

    // saves the tool data to localstorage
    // if no tool is specified then data for all tools is saved
    exports.ControlUtils.exportToolData = function(tool) {
        if (_.isUndefined(tool)) {
            toolList.forEach(exportTool);
        } else {
            exportTool(tool);
        }
    };

    // reads in the tool data from localstorage
    // returns null if nothing is saved there
    exports.ControlUtils.importToolData = function(name) {
        var value = localStorage['toolData-' + name];
        if (value) {
