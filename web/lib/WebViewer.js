...
PDFTron.WebViewer.prototype = {
    ...
    _viewerLoaded: function(iframe) {
        ...
        if (me.options.l) {
            viewerWindow.readerControl.l = me.options.l;
        }
    ...
    _getHTML5OptionsURL: function() {
        ...
        if (options.documentType === 'pdf') {
            acceptPdfUrl();
                ...
                typeof options.initialDoc !== "undefined" && options.initialDoc.slice(-4) === '.pdf') {
            acceptPdfUrl();
        }
    ...
    _correctRelativePath: function(path) {
        ...
        var pattern = /^(https?:\/\/|file:\/\/|\/|blob:|%2F)/i;
    ...
    isChrome: function() {
        ...
        if (window.navigator.userAgent.match(/Chrome\/(.*?) /) && !opera && !maxthon) {
            return true;
        } else {
            return false;
        }
 ...
 * @property {boolean} [enableAnnotations=false] name a boolean to enable annotations.
...
PDFTron.WebViewer.Options = {
    type: "html5,html5Mobile",
    path: '',
    html5Path: "html5/ReaderControl.php",
